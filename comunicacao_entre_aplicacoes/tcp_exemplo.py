#!/usr/bin/env python3
# tcp_exemplo.py - Cliente e servidor TCP simples que enviam e recebem 16 octetos
...
def servidor(interface, port):
    sock = socket.socket(socket.AF_INET, socket.SOCK_STREAM) # Cria socket stream TCP
    ...
    while True:
        ...
        print('  Nome do socket - servidor:', sc.getsockname()) # Visualizar qual porta TCP o socket está usando no local
        print('  Par do socket - cliente:', sc.getpeername()) # Visualizar o endereço do cliente ao qual um soquete conectado está vinculado
